...
void cGame::initialise(SDL_Window* theSDLWND, SDL_Renderer* theRenderer)
{
	...
	texturesToUse = { "Images/Objects/boulder1.png", "Images/Objects/boulder2.png", "Images/Objects/TNT.png", "Images/Objects/PlayerCharacter.png", "Images/Objects/Arrow.png", "Images/Backgrounds/Background.png" };
	...
	theTextureManager->addTexture("TitleTxt", theFontManager->getFont("TheLastCall")->createTextTexture(theRenderer, gameTextList[0], SOLID, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
	theTextureManager->addTexture("ScoreTxt", theFontManager->getFont("tiza")->createTextTexture(theRenderer, gameTextList[1], SOLID, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
	theTextureManager->addTexture("GameOverTxt", theFontManager->getFont("tiza")->createTextTexture(theRenderer, gameTextList[2], SOLID, { 0, 255, 0, 255 }, { 0,0,0,0 }));
	...
	for (int bould = 0; bould < 3; bould++)
	{
		...
		theBoulders[bould]->setBoulderVelocity({ static_cast<int>((3.0f), (3.0f)) });
...
//		theButtonManager->getBtn("pause_btn")->render(theRenderer, &theButtonManager->getBtn("pause_btn")->getSpriteDimensions(), &theButtonManager->getBtn("pause_btn")->getSpritePos(), theButtonManager->getBtn("pause_btn")->getSpriteScale());
		...
		theTextureManager->addTexture("Score", theFontManager->getFont("tiza")->createTextTexture(theRenderer, strScore.c_str(), SOLID, { 0, 255, 0, 255 }, { 0, 0, 0, 0 }));
...
void cGame::update(double deltaTime)
{
	...
	*/
	...
	for (vector<cTNT*>::iterator tntIterator = theTNTs.begin(); tntIterator != theTNTs.end(); ++tntIterator)
	{
		for (arrowIterator = theArrows.begin(); arrowIterator != theArrows.end(); ++arrowIterator)
		{
	...
	while (SDL_PollEvent(&event))
	{
		...
		case SDL_KEYDOWN:
			...
			case SDLK_SPACE:
			{
				...
				theTNTs[numTNT]->setSpriteTranslation({ static_cast<int>((2.0f), (2.0f)) });
				...
				theTNTs[numTNT]->setTNTVelocity({ static_cast<int>((2.0f), (2.0f)) });
